...
#pragma mark - 下载
int download(void *ocObj,CURL *curlhandle, const char * remotepath, const char * localpath, long timeout, long tries, const char * account)
{
    ...
    curl_easy_setopt(curlhandle, CURLOPT_USERPWD, account);
    //连接超时设置
...
- (void)startDownload:(NSString *)urlStr locaPath:(NSString *)locaPath account:(NSString *)account password:(NSString *)password{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        download(p,
                 ...
                 5,
                 3,
                 [[NSString stringWithFormat:@"%@:%@",account,password] cStringUsingEncoding:NSUTF8StringEncoding]);
        curl_easy_cleanup(self->currentCurl);
    });
}
